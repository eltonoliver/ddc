<form name="formGeral" id="formGeral" action="controller/act-geral" method="post" enctype="multipart/form-data">
    ...
    <br/>
    <h2 class="separadador">Or&ccedil;amento e Compras</h2>
    <table width="100%" border="0" align="center" class="tabelaForm">
        <tr>
            <td width="15%">Habilitar Orçamentos?</td>
            <td width="85%">
                <input type="radio" id="inOrcamento" name="inOrcamento" value="1" <?php
                if ($geralConfig && $geralConfig[0]["inOrcamento"] == 1) {
                    echo 'checked="checked"';
                } else if (!$geralConfig) {
                    echo 'checked="checked"';
                }
                ?>/> Sim
                <input type="radio" id="inOrcamento" name="inOrcamento" value="0" <?php
                if ($geralConfig && $geralConfig[0]["inOrcamento"] == 0) {
                    echo 'checked="checked"';
                }
                ?>/> Não
            </td>
        ...
        <tr>
            <td width="15%">Habilitar Compras (PagSeguro)?</td>
            <td width="85%">
                <input type="radio" id="inPagSeguro" name="inPagSeguro" value="1" <?php
                if ($geralConfig && $geralConfig[0]["inPagSeguro"] == 1) {
                    echo 'checked="checked"';
                } else if (!$geralConfig) {
                    echo 'checked="checked"';
                }
                ?>/> Sim
                <input type="radio" id="inPagSeguro" name="inPagSeguro" value="0" <?php
                if ($geralConfig && $geralConfig[0]["inPagSeguro"] == 0) {
                    echo 'checked="checked"';
                }
                ?>/> Não
            </td>
        ...
        <tr>
            ...
            <td width="85%"><input name="nmEmailPagSeguro" id="nmEmailPagSeguro" size="60" maxlength="100" style="top:auto" value="<?php echo $geralConfig[0]["nmEmailPagSeguro"]; ?>"/></td>
        </tr>
    </table>
    <br/>            
    <div align="left">
