...
<?php
	//PREPARA�AO DO PAGINADOR
		...
		if (isset($_REQUEST["idPerfil"]) && strlen($_REQUEST["idPerfil"]) > 0){
			...
			$strMenus = "SELECT DISTINCT(idMenu) FROM tb_perfil_menu WHERE idPerfil = ".$db->clean($idPerfil);
			$qryMenusFiltro = $db->query($strMenus);
			$listaMenus = campoMatrizParaLista('',$qryMenusFiltro,'idMenu');
			if(strlen($listaMenus) > 0){
				$filtro .= " AND A.idMenu in (".$listaMenus.")";	
			} else {
				$filtro .=  "AND A.idMenu in (0)";
			}
		} 
		
		
		//Busca o total de registros da consulta nao paginada
		$qrTotal = "SELECT COUNT(A.idMenu) as total_registros FROM tb_menu A WHERE A.idTipoMenu = 1 ".$filtro;
...
?>  
            <form name="formGeral" id="formGeral" action="menu-menus" method="post">
            <h1>Manutenção de Menus</h1>
            <br/>
            	...
            	<tr class="<?php echo $classe; ?>">
                	<td align="center" width="3%">
                    	<img src="<?php echo $url_raiz; ?>admin/img/del.png" 
                        	 onclick="excluirMenu('<?php echo $qryMenus[$i]["idMenu"]; ?>');" 
                             onmouseover="javascript: this.style.cursor='pointer';"
                             title="Excluir"
                             alt="Excluir"/>
                    </td>
                	<td align="center" width="3%">
                    	<a href="cad-menu?idMenu=<?php echo $qryMenus[$i]["idMenu"]; ?>&idMenuPai=<?php echo $qryMenus[$i]["idMenuPai"]; ?>">
                	...
                	<td align="center" width="4%">
						<?php if($qryMenus[$i]["inExibir"] == 1){ ?>
                            <img src="<?php echo $url_raiz; ?>admin/img/bola_verde.png" 
                                 onclick="javascript: location.href='controller/act-menu?acao=Destivar&idMenu=<?php echo $qryMenus[$i]["idMenu"]; ?>';" 
                                 onmouseover="javascript: this.style.cursor='pointer';"
                        ...
                        <?php } else { ?>
                            <img src="<?php echo $url_raiz; ?>admin/img/bola_cinza.png" 
                                 onclick="javascript: location.href='controller/act-menu?acao=Ativar&idMenu=<?php echo $qryMenus[$i]["idMenu"]; ?>';" 
                                 onmouseover="javascript: this.style.cursor='pointer';"
