<?php 
	$qryCategoriasPai = $db->query("SELECT * FROM tb_categoria WHERE idCategoriaPai = 0 AND inTipo <> 3 ORDER BY nmCategoria DESC");
	...
	//QUERY PAGINADA
		...
		if(strlen($idCategoriaPai) > 0){
			$filtro .= ' AND A.idCategoriaPai = '.$db->clean($idCategoriaPai);
		}
		...
		$qryCont = "
			...
			WHERE 		A.inTipo <> 3
			".$filtro." 
			ORDER BY 	B.nmCategoria, A.nmCategoria DESC
			LIMIT 		".$inicio.",".$limite."		
		";
...
<form name="formGeral" id="formGeral" action="menu-categorias" method="post">
...
<?php */?>        
      <tr>
            <td width="7%">Filtrar por Classifica&ccedil;&atilde;o</td>
            <td>
              <select name="inTipo" id="inTipo"  style="top:auto">
                    <option value="">Todos</option>
                    <option value="1" <?php if($inTipo == 1){ echo 'selected'; } ?>>Categoria</option>
                    <option value="2" <?php if($inTipo == 2){ echo 'selected'; } ?>>Tag</option>
              </select>
            </td>
        </tr>
    </table>
...
<table width="100%" border="0" align="center" class="tbLista">
    ...
    <tr class="<?php echo $classe; ?>">
        <td align="center" width="4%">
            <?php if($_SESSION["PERFIL"] != 1 && $qryCategorias[$i]["inTipo"] != 1){ ?>
                <img src="<?php echo $url_raiz; ?>admin/img/del.png" 
                     onclick="excluirCategoria('<?php echo $qryCategorias[$i]["idCategoria"]; ?>');" 
        ...
        <td align="center" width="5%">
            <?php if($_SESSION["PERFIL"] != 1 && $qryCategorias[$i]["inTipo"] != 1){ ?>
                <img src="<?php echo $url_raiz; ?>admin/img/editar2.png" 
                     onclick="javascript: location.href='cad-categoria?idCategoria=<?php echo $qryCategorias[$i]["idCategoria"]; ?>&idCategoriaPai=<?php echo $qryCategorias[$i]["idCategoriaPai"]; ?>';" 
                     onmouseover="javascript: this.style.cursor='pointer';"
                     title="Editar"
                     alt="Editar"/> 
