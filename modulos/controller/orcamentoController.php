...
switch ($acao) {
    case "Adicionar":
        ...
        $query = "SELECT idPedido FROM tb_pedido WHERE idTipoPedido = 1 AND idSessao = '" . $_SESSION["ID_SESSAO"] . "' ORDER BY idPedido DESC LIMIT 1";
        ...
        if ($qryVerifica[0]["idPedido"] <= 0) { //1.1 Se não achou nenhum pedido em aberto para esta sessão, cria um pedido.
            ...
            $GLOBALS["ID_SESSAO"] = $_SESSION["ID_SESSAO"];
            ...
            if ($qryVerificaProduto) {//Se achou este produto no pedido do cliente, retorna uma mensagem
                echo "
								<script type='text/javascript'>
									...
									location.href='../?pg=orcamento.php';
								</script>
							";
    ...
    case "Excluir":
        ...
        if ($_REQUEST["retorno"] == 'admin') {
            $caminho = '../admin/cad-pedido?idPedido=' . $_REQUEST["pedido"];
        } else {
            $caminho = '../?pg=orcamento.php';
        }
    ...
    case "AdicionarAdmin":
        ...
        $query = "	SELECT idConteudo as idProduto,
							...
							WHERE	idTipoConteudo = 31
							AND		inPublicar = 1
							AND		idConteudo = '" . (int) $_REQUEST["idProduto"] . "'";
        ...
        $query = "
					...
					SELECT 	idProduto
					FROM 	tb_pedido_produto 
					WHERE	idPedido = '" . (int) $_REQUEST["idPedido"] . "'
					AND		idProduto = '" . (int) $_REQUEST["idProduto"] . "'
        ...
        if ($qryVerificaProduto) {//Se achou este produto no pedido do cliente, retorna uma mensagem
            $caminho = '../admin/cad-pedido?idPedido=' . (int) $_REQUEST["idPedido"];
            ...
            $caminho = '../admin/cad-pedido?idPedido=' . $_REQUEST["idPedido"];
    ...
    case "concluirPS":
        ...
        $query = "
				...
				FROM 	tb_pedido 
				WHERE	idSessao		= '" . $GLOBALS["ID_SESSAO"] . "'
				AND		idTipoPedido = 1
        ...
        $subject = '[' . $qryRodape[0]["nmEmailPrincipal"] . '] PEDIDO DE COMPRA';
        ...
        $message = 'Código do Pedido: ' . $pedido . '<br/>
							...
							Data: ' . $data . '<br/>
							Hora: ' . $hora . '<br/><br/>
							Comentários: ' . $_POST["nmObservacoesPedido"] . '<br/>
							-  Para maiores informações, acesse sua conta do PagSeguro e busque por este código de pedido na opção EXTRATO DE TRANSAÇÕES.';
        ...
        echo "
				<script type='text/javascript'>
					location.href='../?PS=" . $_GET["id_pagseguro"] . "&id=" . base64_encode($pedido) . "&pg=retornoOrcamento.php';
				</script>
			";
    ...
    case "concluir":
        ...
        //2.1 Busca o ID do pedido pela sessão atual 
        ...
        $qry = "
						...
						WHERE 		A.idSessao = '" . $_SESSION["ID_SESSAO"] . "' 
						AND			C.idTipoConteudo = 31
        ...
        $to = $qryRodape[0]["nmEmailPrincipal"];
        $subject = '[' . $geralConfig[0]["nmTituloSite"] . '] PEDIDO DE ORÇAMENTO';
  ...
  <tr>
    <td><p>Você está recebendo este e-mail porque um Pedido de Orçamento que foi feito em nome de ' . $_POST["cliente_nome"] . ', em ' . $data . ', às ' . $hora . ', no site <a href="' . $url_raiz . '">' . $geralConfig[0]["nmTituloSite"] . '</a> (' . $url_raiz . ').</p>
	<div align="center">
  ...
  <tr>
    <td><p><br />
      <strong>Código do Pedido:</strong> ' . $pedido . '<br />
      <strong>Tipo / Situação:</strong> Orçamento Solicitado<br /><br />
  ...
  <tr>
    <td><div align="center">
      ...
      <p align="center">DÚVIDAS? Acesse <a href="' . $url_raiz . '?pg=contato.php">' . $url_raiz . '?pg=contato.php</a></p></td>
  </tr>
  <tr>
    <td>
	<div align="center">
      ...
      <p align="center">Este é um e-mail automático disparado pelo sistema. Favor    não respondê-lo, pois esta conta não é monitorada. </p></td>
  </tr>
  <tr>
    ...
    case "Finalizar":
        ...
        $subject = 'RES: [' . $geralConfig[0]["nmTituloSite"] . '] PEDIDO DE ORÇAMENTO';
  ...
  <tr>
    <td><p>Você está recebendo este e-mail em respostao a um Pedido de Orçamento foi feito em nome de ' . $qryCliente[0]["nmCliente"] . ', em ' . date('d/m/Y - H:i:s', strtotime($qryPedido[0]["dtDataPedido"])) . ', no site <a href="' . $url_raiz . '">' . $geralConfig[0]["nmTituloSite"] . '</a> (' . $url_raiz . ').</p>
	<div align="center">
      <hr size="2" width="100%" noshade="noshade" align="center" />
    </div>
	</td>
  ...
  <tr>
    <td><p><br />
      <strong>Código do Pedido:</strong> ' . $pedido . '<br />
      <strong>Tipo / Situação:</strong> Orçamento Finalizado<br />
      <strong>Orçamento Elaborado por:</strong> ' . $_SESSION["NOME"] . '<br /><br />
  ...
  <tr>
    <td background="https://p.simg.uol.com.br/out/pagseguro/i/email/deg_rodapemail.gif"><p>&nbsp;</p></td>
        ...
        //Retorno
        $caminho = '../admin/cad-pedido?idPedido=' . $qryPedido[0]["idPedido"];
