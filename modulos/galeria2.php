...
if (!isset($id) || !$id) {
        ...
        <?php foreach ($qry as $item) { ?>
            <article class="grid-item">
                <a href="galeria/id/<?php echo $item["idGaleria"]; ?>">
                    <img id="img-galeria" src="timthumb.php?src=<?php echo $url_raiz; ?>arquivos/enviados/galeria/<?php echo $item["idGaleria"]; ?>/<?php echo $item["nmImagem"]; ?>&w=480&h=480" />
    ...
    //Calcula os registros inicial e final as serem pesquisados no banco de dados
    $inicio = ($pagina * $limite) - $limite;
    $str = "SELECT g.idGaleria,g.nmGaleria,g.dtDataGaleria,g.nmDescricao,g.nmLocal,i.nmImagem FROM tb_galeria g left join tb_galeria_imagem i on i.idGaleria=g.idGaleria WHERE g.inAtivo=1 and g.idGaleria=" . $id . " order by g.idGaleria LIMIT " . $inicio . "," . $limite;
    $qry = $db->query($str);
    ?>
    <div class="grid grid-galeria">
        <?php foreach ($qry as $foto) { ?>
            <article class="grid-item">
                <a data-modal='
                   ...
                   <img src="timthumb.php?src=<?php echo $url_raiz; ?>arquivos/enviados/galeria/<?php echo $foto["idGaleria"]; ?>/<?php echo $foto["nmImagem"]; ?>&w=770&h=430" linkCurto="timthumb.php?src=<?php echo $url_raiz . "arquivos/enviados/image/" . $foto["idGaleria"] . "/" . $foto["nmImagem"]; ?>&w=770&h=430" />
                   <h3><?php echo $foto["nmGaleria"]; ?></h3>
                   <p>
                   <?php echo $foto["nmDescricao"]; ?>
                   </p>
                   </div>
                   ' class="fancybox" rel="group" href="timthumb.php?src=<?php echo $url_raiz ?>arquivos/enviados/image/<?php echo $foto["idGaleria"]; ?>/<?php echo $foto["nmImagem"]; ?>&w=770&h=430">
                    <img src="timthumb.php?src=<?php echo $url_raiz; ?>arquivos/enviados/galeria/<?php echo $foto["idGaleria"]; ?>/<?php echo $foto["nmImagem"]; ?>&w=480&h=480" linkCurto="<?php echo $url_raiz . "arquivos/enviados/image/" . $foto["idGaleria"] . "/" . $foto["nmImagem"]; ?>" />
